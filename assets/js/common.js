...
{if(isNaN(value)){return value;}else{decima=Math.pow(10,decima);var rndvalus = Math.round(value * decima) / decima;
...
function easyFaqPost(s){"logged"==s?(question=$("#faques").val(),s="logged"):(question=$("#updques").html(),s="guest"),tags="",category="",desc="",faq_status="1";var e={process_type:"add_faq",question:question,tags:tags,category:category,desc:desc,user_type:s,faq_status:faq_status},t=make_faq_request(e);"success"==t.process?(redirect=t.msg,window.location=redirect):(aasrrorMes=t.msg,$("#faqerr").html(errorMes))}
function faqpostchecks(){$("#faqerr").html("");var t=$.trim($("#faques").val());if(""==t)return $("#faqerr").html("<p style='color:red;hright:auto;margin-bottom: -6px;'> Enter your question</p>"),$("#faques").focus(),!1;var r=isHTMLContains(t),e=replaceURLWithHTMLLinks(t);if(0==r&&1==e){var o=document.getElementById("faqformpost");o.submit()}else $("#faqerr").html("<p style='color:red;hright:auto;margin-bottom: -6px;'>Self promotion and external links are not accepted in your answers.</p>")}
